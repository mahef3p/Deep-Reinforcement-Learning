...
class Agent:
    ...
    def learn(self, experiences, gamma):
        ...
        # Double DQN
        # Get the positions of max predicted Q values (for next states) from local model
        with torch.no_grad():
            actions_positions = self.qnetwork_local(next_states).detach().argmax(1).unsqueeze(1)
        # Get max predicted Q values (for next states) from target model
        Q_targets_next = self.qnetwork_target(next_states).detach().gather(1, actions_positions)
