...
def train_ddpg(dev, weights_file_actor, weights_file_critic, n_episodes=1000, max_t=1000):
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        for t in range(max_t):
            ...
            if np.any(dones):
                break 
        mean_score = np.mean(current_scores)  # current averaged score of all the agents
        scores.append(mean_score)
        scores_window.append(mean_score)
        averages.append(np.mean(scores_window))
        if (i_episode % 100 != 0):
            print('\rEpisode {}\tScore: {:.2f}\tAverage Score: {:.2f}'.format(i_episode, mean_score, averages[i_episode-1]), end="")
        else:
            print('\rEpisode {}\tScore: {:.2f}\tAverage Score: {:.2f}'.format(i_episode, mean_score, averages[i_episode-1]))
        if (averages[i_episode-1] >= 30.0):
...
def test(dev, weights_file_actor, weights_file_critic, n_episodes=100, max_t=1000):
    """Test the environment with the parameters stored in the weights files
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        scores.append(mean_score)
        if (i_episode % 100 != 0):
            print('\rEpisode {}\tScore: {:.2f}\tAverage Score: {:.2f}'.format(i_episode, mean_score, np.mean(scores)), end="")
        else:
            print('\rEpisode {}\tScore: {:.2f}\tAverage Score: {:.2f}'.format(i_episode, mean_score, np.mean(scores)))
